...
const TransactionForm: React.FC = () => {
  ...
  const [selectedType, setSelectedType] = useState<"Sortie" | "Entrée">(
    "Sortie",
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedType(e.target.value as "Sortie" | "Entrée")
            }
          >
        ...
        {/* Tiers dynamique (Provenance/Destination selon Règle 4.2) */}
        <div>
          <label className="block text-xs font-medium text-slate-700 mb-1">
            <span className="flex items-center gap-1">
              <User size={12} />
              {selectedType === "Entrée" ? "Provenance" : "Destination"}
            </span>
          </label>
          <input
            type="text"
            ...
            className="w-full p-2.5 border border-slate-300 rounded-lg text-sm outline-none focus:ring-1 focus:ring-blue-500"
            required
          />
        </div>
        {/* Banque */}
